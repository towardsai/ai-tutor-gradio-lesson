#!/usr/bin/env python3
"""Build the ai_tutor_knowledge vector store, so the app loads it instead of
re-embedding. This is the same script that built the prebuilt stores hosted in
the course dataset (towardsai-tutors/full-stack-ai-engineering-data) — use it
to rebuild the store with either embedding provider, or point --input at your
own JSONL to serve your own data.
...
Each run writes prebuilt_stores/<config>/{chroma/, manifest.json} plus a
<config>-<dataset-hash>.zip. The app (app.py) loads the 1536d store matching
its EMBED_PROVIDER. A rate-limited or interrupted run is safe: rerun the same
command and batches already in the store are skipped, not re-embedded. A store
left over from a DIFFERENT chunking recipe is detected (manifest recipe, then
stored-text comparison) and refused — delete its folder first; stale and fresh
chunks never mix.
...
DATASET_BASE = "https://huggingface.co/datasets/towardsai-tutors/full-stack-ai-engineering-data/resolve/main/"
...
parser.add_argument("--input", type=Path, default=REPO / "ai_tutor_knowledge.jsonl",
                    help="course corpus JSONL (auto-downloaded from the course dataset if missing)")
parser.add_argument("--output", type=Path, default=REPO / "prebuilt_stores")
...
# --- fetch the course corpus if it isn't already next to this repo -------------
if not args.input.exists():
    print(f"{args.input} not found — downloading from the course dataset...")
    with requests.get(DATASET_BASE + "ai_tutor_knowledge.jsonl", stream=True, timeout=600) as response:
